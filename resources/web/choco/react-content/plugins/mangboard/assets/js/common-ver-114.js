...
function set_category_data(data, id, value) {
  if (typeof data !== "undefined") {
    ...
    if (
      typeof mb_languages["selectbox" + index] != "undefined" &&
      mb_languages["selectbox" + index] != ""
    )
      jQuery("#" + id).append(
        '<option value="">' + mb_languages["selectbox" + index] + "</option>"
      );
...
function moveViewPage(pid, board_name, page) {
  var param = "vid=" + pid;
  if (typeof board_name !== "undefined" && board_name != "")
    param = param + "&board_name=" + board_name;
...
function openWindow(url, name, option) {
  ...
  if (typeof mb_hybrid_app === "undefined" || mb_hybrid_app == "") {
    if (typeof option === "undefined")
      option =
        "width=600,height=450,toolbar=no,location=no,status=no,menubar=no,top=200,left=300,scrollbars=no,resizable=no";
    if (
      typeof mb_options !== "undefined" &&
      typeof mb_options["device_type"] !== "undefined" &&
      mb_options["device_type"] == "mobile"
    ) {
...
function category_select(index) {
  if (index == 0) {
    set_category_data(
      category_data,
      mb_options["board_name"] + "_category1",
      mb_categorys["value1"]
    );
    if (
      mb_categorys["value1"] != undefined &&
      mb_categorys["value1"] != "" &&
      jQuery("#" + mb_options["board_name"] + "_category1 option")
        .filter(":selected")
        .val() != ""
    ) {
      set_category_data(
        ...
        mb_options["board_name"] + "_category2",
        mb_categorys["value2"]
      );
    } else jQuery("#" + mb_options["board_name"] + "_category2").hide();
    if (
      mb_categorys["value2"] != undefined &&
      mb_categorys["value2"] != "" &&
      jQuery("#" + mb_options["board_name"] + "_category2 option")
        .filter(":selected")
        .val() != ""
    ) {
      set_category_data(
        ...
        mb_options["board_name"] + "_category3",
        mb_categorys["value3"]
      );
...
function sendBoardFileData(file_pid, file_name) {
  var data =
    "mode=file&board_action=file_download&board_name=" +
    mb_options["board_name"] +
    "&file_pid=" +
    file_pid +
    "&file_name=" +
...
function sendBoardFileDataHandler(response, state) {
  if (response.state == "success") {
    if (
      ...
      if (
        mb_hybrid_app == "ios" &&
        typeof response.data.file_path2 !== "undefined"
      ) {
        var file_url = mb_urls["base"];
        file_url = file_url.replace(
          "/react-content/plugins",
          "/react-content/uploads"
        );
        homeSendMessage({
          mode: "FILE_DOWNLOAD",
          value: file_url + response.data.file_path2,
          name: mb_selectFileName,
        });
      } else {
        downloadFile(
          mb_urls["file"],
          "path=" + encodeURIComponent(response.data.file_path)
        );
      }
    }
  } else {
...
function sendTabReload(data, idx) {
  ...
  if (jQuery("input[name=category" + idx + "]"))
    jQuery("input[name=category" + idx + "]").val(data);
  if (idx == 1 && data == "") {
    if (jQuery("input[name=category2]"))
      jQuery("input[name=category2]").val(data);
    if (jQuery("input[name=category3]"))
      jQuery("input[name=category3]").val(data);
  } else if (idx == 2 && data == "") {
    if (jQuery("input[name=category3]"))
      jQuery("input[name=category3]").val(data);
  }
...
function sendListTemplateData(data) {
  ...
  if (typeof data.board_name === "undefined" || data.board_name === "undefined")
    board_name = mb_options["board_name"];
  ...
  var params = jQuery(
    "#" + listTemplateBoard + "_form_board_search"
  ).serialize();
  if (jQuery("#" + listTemplateBoard + "_form_board_search2").length > 0)
    params =
      params +
      "&" +
      jQuery("#" + listTemplateBoard + "_form_board_search2").serialize();
  params =
    params +
    "&" +
    jQuery("#" + listTemplateBoard + "_form_board_list").serialize() +
    "&board_action=load";
  ...
  if (typeof data.category !== "undefined") {
    ...
    typeof data.page_type !== "undefined" &&
    data.page_type == "ajax"
  ) {
    if (
      typeof mb_categorys["value1"] !== "undefined" &&
      mb_categorys["value1"] != ""
    ) {
      params =
        params + "&category1=" + encodeURIComponent(mb_categorys["value1"]);
      if (
        typeof mb_categorys["value2"] !== "undefined" &&
        mb_categorys["value2"] != ""
      ) {
        params =
          params + "&category2=" + encodeURIComponent(mb_categorys["value2"]);
        if (
          typeof mb_categorys["value3"] !== "undefined" &&
          mb_categorys["value3"] != ""
        ) {
          params =
            params + "&category3=" + encodeURIComponent(mb_categorys["value3"]);
        }
      }
    }
  }
  ...
  if (listTemplateCheck) {
    if (response.state == "success") {
      ...
      if (response.data["body"])
        jQuery("#" + listTemplateBoard + "_board_body").append(
          response.data["body"]
        );
...
function template_combo_handler(type, obj, name) {
  ...
  var combo_wrapper = jQuery(obj)
    .closest(".mb-combo-wrapper")
    .find(".mb-combo-items");
  combo_wrapper.children().hide();
  combo_wrapper.find(":input").prop("disabled", true);
  combo_wrapper.find(".mb-combo-" + name + "-" + value).show();
  combo_wrapper
    .find(".mb-combo-" + name + "-" + value)
    .find(":input")
    .prop("disabled", false);

  if (
    typeof template_combo_hide[name] !== "undefined" &&
  ...
  if (
    combo_wrapper.find(
      ".mb-combo-" + name + "-" + value + " .mb-combo-box-hide-element"
    ).length > 0
  ) {
    var combo_hide = combo_wrapper
      .find(".mb-combo-" + name + "-" + value + " .mb-combo-box-hide-element")
      .val();
    if (combo_hide != "") {
...
function template_combo_display_check(type, name, value) {
  if (typeof mb_options["board_name"] !== "undefined" && value != "") {
    ...
    for (var i = 0; i < items.length; i++) {
      if (type == "hide") {
        jQuery("tr#mb_" + item_name + "_tr_" + items[i])
          .find(":input")
          .prop("disabled", true);
...
function checkEnterKey(callback, param) {
  if (event.keyCode == 13) {
    if (typeof param === "undefined") callback();
    else callback(param);
  }
}
var openTarget;
...
function sendContentDataHandler(response, state) {
  if (response.state == "success") {
    ...
    content_html =
      ...
      '<td colspan="' +
      colspan +
      '"><div class="mb-open-slide" style="display:none"><div class="mb-open-content">' +
      response.data +
      "</div></div></td></tr>";
...
function moveBodyScrollPosition(name, top, time) {
  ...
  jQuery("html, body").animate(
    { scrollTop: jQuery(name).offset().top - top },
    time
  );
}
...
function checkSendApiURL() {
  if (
    typeof mb_urls["board_api"] === "undefined" &&
    typeof mb_urls["template_api"] === "undefined"
  ) {
